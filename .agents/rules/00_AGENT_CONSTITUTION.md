...
9. **Mandatory 'Thumbnail BG/' Template & Bilingual Rule**: All newly created thumbnails MUST be composited ON TOP OF the user's authentic templates from `Thumbnail BG/` using the Chromium HarfBuzz engine (`build_official_bg_thumbnails.py`). Plain artificial gradients are strictly prohibited. **English Content Rule:** For English posts (English 1st/2nd Paper, Grammar, Suggestions), the thumbnail MUST contain BOTH English and Bengali (Primary English Title/Keyword + Clear Bengali Hook/Subtitle), and the post content must strictly preserve and feature the target English keywords for SEO ranking. For Bengali posts, thumbnails remain primarily in Bengali.
