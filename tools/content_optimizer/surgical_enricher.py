#!/usr/bin/env python3
"""
Surgically enrich each function in batch_reviver.py to ensure all 10 posts are 1,300 - 1,800 words.
"""
...
# Dictionary of specific section injections for each builder
ENRICHMENTS = {
...
# Split by def build_post_
parts = re.split(r'(?=\ndef build_post_)', text)
new_parts = []

for part in parts:
    matched_fn = None
    for fn_name in ENRICHMENTS:
        if part.startswith(f"\ndef {fn_name}()") or part.startswith(f"def {fn_name}()"):
            matched_fn = fn_name
            break
    
    if matched_fn:
        injection = ENRICHMENTS[matched_fn]
        # Insert injection right before the last closing </div>\n\"\"\" or before schema
        if '<script type="application/ld+json">' in part:
            part = part.replace('<script type="application/ld+json">', injection + '\n  <script type="application/ld+json">', 1)
            print(f"Surgically injected into {matched_fn}")
        else:
...
print("Surgical enrichment finished!")
