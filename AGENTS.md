...
5. **Mandatory `<!--more-->` Jump Break (Strictly AFTER Hero Image):** ব্লগারে পোস্টের মূল ব্যানার বা হিরো ইমেজ (`<img>`) সবার উপরে থাকবে এবং তার নিচে ভূমিকা প্যারাগ্রাফের ঠিক পরেই `<!--more-->` ট্যাগ বসাতে হবে। কোনো অবস্থাতেই হিরো ইমেজের পূর্বে `<!--more-->` বসানো যাবে না। ইমেজের পূর্বে মোর ট্যাগ দিলে ব্লগারে আরএসএস ফিড থেকে ছবি বাদ পড়ে যায় এবং হোমপেজ/রিলেটেড পোস্ট উইজেটে ব্রোকেন ধূসর ক্যামেরা আইকন তৈরি হয়।
...
8. **Bilingual Thumbnail Governance:** পোস্ট যদি ইংরেজিতে হয় তবে থাম্বনেইলের সমস্ত টেক্সট (টাইটেল, সাবটাইটেল, ক্যাটাগরি ব্যাজ ও এডিশন ট্যাগ) ১০০% ইংরেজিতে হতে হবে। পোস্ট যদি বাংলায় হয় তবে থাম্বনেইল ১০০% বাংলায় হবে। কোনো অবস্থাতেই ভাষা মিশ্রণ করা যাবে না।
...
22. **Theme Native CSS Primacy & Zero-Bloat Minimalist Post Styling (ডিফল্ট থিম সিএসএস অগ্রাধিকার ও নিরর্থক কোড বর্জন নীতিমালা):**
    - **ইন-পোস্ট `<style>` ব্লট সম্পূর্ণ নিষিদ্ধ:** প্রতিটি পোস্টের ভেতর বিশাল `<style>` ব্লক দিয়ে ফন্ট ইম্পোর্ট (`@font-face`), `.htbd-post-wrapper`, সাধারণ প্যারাগ্রাফ স্টাইল বা অতিরিক্ত সিএসএস কোড লেখা সম্পূর্ণ নিষিদ্ধ। ব্লগারের মূল থিম (FlexSpot Premium) ইতিমধ্যে সোলায়মানলিপি ফন্ট, লাইন-হাইট, হেডিং ও রেসপনসিভনেস সাইটজুড়ে গ্লোবালি হ্যান্ডেল করে।
    - **থিমের নিজস্ব সেমান্টিক ট্যাগ ও ক্লাসের বাধ্যতামূলক ব্যবহার:**
      * হেডিং: সাধারণ `<h2>`, `<h3>`, `<h4>` (কোনো অতিরিক্ত ক্লাস ছাড়া)।
      * প্যারাগ্রাফ ও লিস্ট: সাধারণ `<p>`, `<ul>`, `<ol>`, `<li>`।
      * টেবিল: সাধারণ `<table>`, `<thead>`, `<tbody>`, `<tr>`, `<th>`, `<td>` (থিম নিজে থেকেই রেসপনসিভ স্ক্রোল র‍্যাপার ও মার্জিত বর্ডার দেয়)।
      * সূচিপত্র (TOC): থিমের নিজস্ব শর্টকোড `<strike>#title=(সূচিপত্র) (toc)</strike>` অথবা সেমান্টিক কন্টেইনার `<div class="tociki-pro">...</div>` ব্যবহার করতে হবে।
      * অ্যালার্ট ও কলআউট বক্স: থিমের নিজস্ব শর্টকোড বা ক্লাস `<div class="alert-message passed">...</div>` (অথবা `success`, `warning`) ব্যবহার করতে হবে।
      * কোড ও কমান্ড বক্স: থিমের নিজস্ব `<pre class="code-box">...</pre>`।
      * বাটন: থিমের নিজস্ব `.main-button.button.sp-bt` ক্লাস।
    - **কাস্টম সিএসএস ব্যতিক্রম:** যদি এমন কোনো বিশেষ উপাদান তৈরি করতে হয় যা মূল থিমে নেই (যেমন: সাইলো সিরিজ নেভিগেশন কার্ড বা স্টেপ কার্ড), তবেই কেবল সেই উপাদানের জন্য অত্যন্ত সংক্ষিপ্ত ও ক্লিন ইনলাইন সিএসএস ব্যবহার করা যাবে।
    - **চড়া রঙ বর্জন ও পাঠের পরম আরাম (Zero Colorful Glitz & High Legibility):** ইউজারের স্পষ্ট নির্দেশনা অনুযায়ী অতিরিক্ত রঙিন কোনো কিছু তৈরি করা সম্পূর্ণ নিষিদ্ধ। স্টাইল হবে পাঠ্যপুস্তকের মতো শান্ত ও পরিচ্ছন্ন। অফ-হোয়াইট ব্যাকগ্রাউন্ড, সূক্ষ্ম স্লেট বর্ডার (`#e2e8f0`), গাড় কালো টেক্সট (`#0f172a`, `#202124`) ও পরিমিত হালকা অ্যাকসেন্ট নিশ্চিত করতে হবে, যাতে ব্যবহারকারী পড়তে পরম স্বস্তি পান।
